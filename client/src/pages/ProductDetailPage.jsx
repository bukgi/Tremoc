...
const ProductDetailPage = () => {
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
      ...
      {toast && (
        <div
          className={`fixed top-6 right-6 z-50 flex items-center gap-3 px-5 py-4 rounded-xl shadow-2xl text-white font-medium text-sm transition-all duration-300 animate-slideUp max-w-sm ${
      ...
      <nav className="flex items-center gap-2 text-sm mb-8 animate-fadeIn">
        <Link to="/" className="text-muted hover:text-forest transition-colors">
          Trang chủ
        </Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
        ...
        >
          ...
          <h1 className="text-3xl lg:text-4xl font-bold text-slate-dark mt-2 mb-3">
            {product.name}
          </h1>
          <p className="text-3xl font-bold text-forest mb-3">
          ...
          <div className="bg-mint rounded-2xl p-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                {
                  icon: Leaf,
                  value: product.impact.plastic,
                  label: "Nhựa giảm thiểu",
                  color: "text-green-600",
                },
                {
                  icon: TreePine,
                  value: product.impact.co2,
                  label: "CO₂ giảm",
                  color: "text-emerald-600",
                },
                {
                  icon: Droplets,
                  value: product.impact.water,
          ...
          {/* Share Section */}
          <div
            className="mt-8 bg-white rounded-2xl p-6 border border-border animate-slideUp"
            style={{ animationDelay: "400ms", animationFillMode: "both" }}
          >
            <div className="flex items-center gap-2 mb-4">
              <Share2 size={20} className="text-forest" />
              <h3 className="text-base font-semibold text-slate-dark">
                Chia sẻ sản phẩm
              </h3>
            </div>
            <div className="flex flex-wrap gap-3">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-mint hover:bg-mint/80 text-slate-dark font-medium text-sm rounded-xl transition-all duration-200 hover:shadow-md"
              >
                {shareToast === true ? (
                ...
                {shareToast === true ? "Đã sao chép!" : "Sao chép liên kết"}
              </button>
              ...
              <a
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-blue-50 hover:bg-blue-100 text-blue-600 font-medium text-sm rounded-xl transition-all duration-200 hover:shadow-md"
              >
                <svg
                  width="16"
                  height="16"
              ...
              {/* Zalo Share */}
              <a
                href={`https://zalo.me/share?u=${encodeURIComponent(window.location.href)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 px-4 py-2.5 bg-blue-50 hover:bg-blue-100 text-blue-700 font-medium text-sm rounded-xl transition-all duration-200 hover:shadow-md"
              >
                <svg
                  width="16"
                  height="16"
                  ...
                  fill="currentColor"
                >
                  <path d="M12.49 10.272v-.45h1.347v6.322h-.77a.576.576 0 01-.577-.573v-5.299zm-2.63 4.173h.003a1.771 1.771 0 01-1.777-1.769v-1.246c0-.977.795-1.77 1.774-1.77h.003c.98 0 1.774.793 1.774 1.77v1.246a1.771 1.771 0 01-1.777 1.769zm9.937-2.249h-1.587v1.519a.56.56 0 01-.558.562h-.002a.56.56 0 01-.558-.562v-1.519h-1.588a.56.56 0 01-.558-.562v-.002c0-.31.25-.56.558-.56h1.588V9.526c0-.31.25-.56.558-.56h.002c.308 0 .558.25.558.56v1.546h1.587c.308 0 .558.25.558.56v.002a.56.56 0 01-.558.562z" />
                </svg>
                Zalo
              </a>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {relatedProducts.map((rp, i) => (
              <Link
                key={rp.id}
                to={`/product/${rp.id}`}
                className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 group border border-border/50 animate-slideUp"
                style={{
                  animationDelay: `${i * 100}ms`,
                  animationFillMode: "both",
                }}
              >
                <div className="aspect-square bg-bg-off overflow-hidden">
                  <img
                    src={rp.image}
                    alt={rp.name}
                ...
                <div className="p-4">
                  ...
                  <h3 className="text-sm font-semibold text-slate-dark mt-1 mb-1 group-hover:text-forest transition-colors line-clamp-2">
                    {rp.name}
                  </h3>
